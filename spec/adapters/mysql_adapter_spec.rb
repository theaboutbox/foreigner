...
describe 'MySQL Adapter' do
  ...
  describe 'when altering tables' do
    it 'should add foreign key without options' do
      @adapter.add_foreign_key(:employees, :companies).should eql("ALTER TABLE `employees` ADD CONSTRAINT `employees_company_id_fk` FOREIGN KEY (`company_id`) REFERENCES `companies`(id)")
    end

    it 'should add foreign key with a name' do
      @name = 'favorite_company_fk'
      @adapter.add_foreign_key(:employees, :companies, :name => @name).should eql("ALTER TABLE `employees` ADD CONSTRAINT `#{@name}` FOREIGN KEY (`company_id`) REFERENCES `companies`(id)")
    end

    it 'should add foreign key with a column' do
      @column = 'last_employer_id'
      @adapter.add_foreign_key(:employees, :companies, :column => @column).should eql("ALTER TABLE `employees` ADD CONSTRAINT `employees_last_employer_id_fk` FOREIGN KEY (`#{@column}`) REFERENCES `companies`(id)")
    ...
    it 'should add foreign key with column and a name' do
      @name = 'favorite_company_fk'
      @column = 'last_employer_id'
      @adapter.add_foreign_key(:employees, :companies, :column => @column, :name => @name).should eql("ALTER TABLE `employees` ADD CONSTRAINT `#{@name}` FOREIGN KEY (`#{@column}`) REFERENCES `companies`(id)")
    end
